            ...
            <nav className="navbar">
                ...
                <div className={`nav-links ${isMenuOpen ? 'active' : ''}`}>
                    <div className="action-buttons" style={{ display: 'flex', gap: '1rem' }}>
                        <a href={(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') && !window.navigator.userAgent.includes('ReactSnap') ? 'http://localhost:3001/login' : 'https://consult.koneacademy.io/login'} className="btn-login" onClick={() => setIsMenuOpen(false)}>Login</a>
                        <a href={((window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') && !window.navigator.userAgent.includes('ReactSnap') ? 'http://localhost:3001' : 'https://consult.koneacademy.io') + "/docs?category=code"} target="_blank" rel="noopener noreferrer" className="btn-login" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)' }} onClick={() => setIsMenuOpen(false)}>Docs</a>
                        <a href={(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') && !window.navigator.userAgent.includes('ReactSnap') ? 'http://localhost:5173/' : 'https://www.koneacademy.io/'} className="btn-hub" onClick={() => setIsMenuOpen(false)}>Back to Hub</a>
                    </div>
                </div>
            </nav>
